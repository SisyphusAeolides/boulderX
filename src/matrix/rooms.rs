...
impl RoomRegistry {
    ...
    pub fn get(&self, room_id: &str) -> Option<&MatrixRoom> { self.rooms.get(room_id) }
    pub fn get_mut(&mut self, room_id: &str) -> Option<&mut MatrixRoom> { self.rooms.get_mut(room_id) }
    ...
    pub fn remove(&mut self, room_id: &str) { self.rooms.remove(room_id); }

    pub fn increment_unread(&mut self, room_id: &str) {
        if let Some(r) = self.rooms.get_mut(room_id) { r.unread_count += 1; }
    }

    pub fn clear_unread(&mut self, room_id: &str) {
        if let Some(r) = self.rooms.get_mut(room_id) { r.unread_count = 0; }
    }
}
