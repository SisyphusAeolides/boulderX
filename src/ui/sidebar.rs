...
pub fn build_room_row(
    ...
    if is_active { row.add_css_class("room-row-active"); }
    row.add_css_class("room-row");

    let hbox = gtk::Box::builder()
        .orientation(gtk::Orientation::Horizontal)
        .spacing(6).margin_start(8).margin_end(8).margin_top(4).margin_bottom(4)
        .build();
    ...
    let vbox = gtk::Box::builder()
        .orientation(gtk::Orientation::Vertical).spacing(1).hexpand(true).build();
    let name_label = gtk::Label::builder()
        .label(name).halign(gtk::Align::Start)
    ...
    let s = sender.clone(); let ch = name.to_string();
    ...
    if matches!(protocol, Protocol::Irc) && channels::is_channel_target(name) {
        ...
        let s2 = sender.clone(); let ch2 = name.to_string();
...
pub fn section_header(label: &str) -> gtk::ListBoxRow {
    let lbl = gtk::Label::builder()
        ...
        .margin_start(10).margin_top(10).margin_bottom(2).build();
