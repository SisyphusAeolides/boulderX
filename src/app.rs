...
impl AppModel {
    ...
    pub fn sync_account_for_server(&mut self, server: &str) {
        self.accounts.insert(server.to_string(), crate::config::ServerAccount {
            nick: self.nickname.clone(),
            password: self.password.clone(),
            service: self.account_service.clone(),
            auth_method: self.auth_method.clone(),
    ...
    pub fn message_style(&self, user: &str, body: &str) -> chat_view::LineStyle {
        if user == "System" { return chat_view::LineStyle::System; }
        let clean = Self::normalized_nick(user);
        if clean.eq_ignore_ascii_case(&self.nickname) { return chat_view::LineStyle::SelfMsg; }
        if body.contains(&self.nickname) { return chat_view::LineStyle::Mention; }
    ...
    pub fn notify_kind(&self, channel: &str, style: chat_view::LineStyle) -> NotifyKind {
        if !channels::is_channel_target(channel) { NotifyKind::DirectMessage }
        else if style == chat_view::LineStyle::Mention { NotifyKind::Mention }
        else { NotifyKind::Activity }
    }

    pub fn append_message(&mut self, channel: &str, user: &str, body: &str, style: chat_view::LineStyle) {
    ...
    pub fn show_channel_history(&self) {
        let lines: Vec<(String, String, String, chat_view::LineStyle)> = self
            ...
            .map(|h| h.iter().map(|l| (l.timestamp.clone(), l.user.clone(), l.body.clone(), l.style)).collect())
            .unwrap_or_default();
    ...
    pub fn refresh_channels(&self, sender: &ComponentSender<Self>) {
        ...
        // IRC section
        self.channel_box.append(&crate::ui::sidebar::section_header("IRC"));
        let filter = self.channel_filter.to_lowercase();
        let mut irc_channels: Vec<&String> = self.channels.iter().filter(|c| {
    ...
    pub fn refresh_users(&self, sender: &ComponentSender<Self>) {
        ...
        if let Some(users) = self.channel_users.get(&self.active_channel) {
            for user in users {
                ...
                if muted { btn.add_css_class("muted-user"); }
                let s1 = sender.clone();
                let u1 = clean.clone();
...
impl SimpleComponent for AppModel {
    ...
    fn init(_init: Self::Init, root: Self::Root, sender: ComponentSender<Self>) -> ComponentParts<Self> {
        ...
        chat_histories.insert(server_tab.clone(), vec![
            ChatLine { timestamp: ts.clone(), user: "System".to_string(), body: "Ready. Configure server/nick and Connect.".to_string(), style: chat_view::LineStyle::System },
            ChatLine { timestamp: ts.clone(), user: "System".to_string(), body: "Use /join #channel or the join box.".to_string(), style: chat_view::LineStyle::System },
        ]);
    ...
    fn update(&mut self, message: Self::Input, sender: ComponentSender<Self>) {
        match message {
            AppInput::UpdateNickname(nick) => { self.nickname = nick; let s = self.server.clone(); self.sync_account_for_server(&s); }
            AppInput::UpdateServer(srv) => { let c = self.server.clone(); self.sync_account_for_server(&c); self.load_account_for_server(&srv); self.server = srv; }
            ...
            AppInput::IgnoreUser(nick) => {
                ...
                self.append_message(&chan, "System", &format!("Ignoring {}", clean), chat_view::LineStyle::System);
            }
            AppInput::UnignoreUser(nick) => {
                let clean = Self::normalized_nick(&nick);
                self.ignored_users.remove(&clean);
                let chan = self.active_channel.clone();
                self.append_message(&chan, "System", &format!("Unignored {}", clean), chat_view::LineStyle::System);
            }
            AppInput::UserRenamed { old, new } => {
                for list in self.channel_users.values_mut() {
                    for u in list.iter_mut() { if *u == old { *u = new.clone(); } }
                }
                self.refresh_users(&sender);
            }
            AppInput::BrowseChannels => {
                if self.connection != ConnectionState::Connected {
                    self.append_message(SERVER_TAB, "System", "Connect first to browse channels.", chat_view::LineStyle::System);
                    return;
                }
                self.channel_list_results.clear();
                if let Some(tx) = &self.irc_sender {
                    let _ = tx.send(irc::client::prelude::Message::from("LIST"));
                    self.append_message(SERVER_TAB, "System", "Requesting channel list…", chat_view::LineStyle::System);
                }
            }
            ...
            AppInput::ChannelListEnd => {
                ...
                if results.is_empty() {
                    self.append_message(SERVER_TAB, "System", "No channels returned.", chat_view::LineStyle::System);
                    return;
                }
                ...
                let search = gtk::SearchEntry::builder().placeholder_text("Filter…").hexpand(true).build();
                vbox.append(&search);
                ...
                for (name, users, topic) in &sorted {
                    ...
                    let s = sender.clone(); let ch = name.clone(); let dlg = dialog.clone();
                    join_btn.connect_clicked(move |_| { s.input(AppInput::JoinChannel(ch.clone())); dlg.close(); });
                    row.append(&info); row.append(&join_btn);
                    list_box.append(&row);
                ...
                search.connect_changed(move |e| {
                    ...
                    for (row, name, topic) in &rows2 {
                        row.set_visible(q.is_empty() || format!("{} {}", name, topic).to_lowercase().contains(&q));
                    }
                });
                let close = gtk::Button::with_label("Close");
                let d = dialog.clone(); close.connect_clicked(move |_| d.close());
                vbox.append(&close);
            ...
            AppInput::SubmitRegistration { nick, password, email } => {
                ...
                if let Some(tx) = &self.irc_sender {
                    ...
                    self.append_message(SERVER_TAB, "System", &format!("Sent registration for {}.", nick), chat_view::LineStyle::System);
            ...
            AppInput::SubmitVerification { nick, code } => {
                let service = self.account_service.clone();
                if let Some(tx) = &self.irc_sender {
                    let _ = tx.send_privmsg(&service, &format!("VERIFY REGISTER {} {}", nick, code));
                    self.append_message(SERVER_TAB, "System", &format!("Sent VERIFY for {}.", nick), chat_view::LineStyle::System);
                }
            }
            AppInput::UpdateAccountService(s) => { if !s.is_empty() { self.account_service = s; let srv = self.server.clone(); self.sync_account_for_server(&srv); self.persist_settings(); } }
            AppInput::UpdateAuthMethod(m) => { if !m.is_empty() { self.auth_method = m; let srv = self.server.clone(); self.sync_account_for_server(&srv); self.persist_settings(); } }
            AppInput::SendRawPrivmsg { target, msg } => {
                if let Some(tx) = &self.irc_sender { let _ = tx.send_privmsg(&target, &msg); }
            }
            ...
            AppInput::Connect => {
                ...
                self.persist_settings();
                let channels_to_join: Vec<String> = self.channels.iter()
            ...
            AppInput::Disconnect => {
                if let Some(tx) = self.irc_sender.take() {
                    ...
                    self.channel_list_results.clear();
                    self.append_message(SERVER_TAB, "System", "Disconnected.", chat_view::LineStyle::System);
                }
            }
            ...
            AppInput::NetworkConnected(tx) => {
                ...
                self.append_message(SERVER_TAB, "System", &format!("Connected to {} as {}.", self.server, self.nickname), chat_view::LineStyle::System);
                if let Some(email) = self.pending_register_email.take() {
                    let service = self.account_service.clone();
                    let cmd = format!("REGISTER {} {}", self.password, email);
                    let _ = tx.send_privmsg(&service, &cmd);
                    self.append_message(SERVER_TAB, "System", &format!("Sent registration to {}.", service), chat_view::LineStyle::System);
                }
            }
            ...
            AppInput::JoinChannel(target) => {
                if !self.channels.contains(&target) {
                    ...
                    self.chat_histories.entry(target.clone()).or_insert_with(|| vec![
                        ChatLine { timestamp: self.timestamp_prefix(), user: "System".to_string(), body: format!("Tracking {}", target), style: chat_view::LineStyle::System }
                    ]);
                    self.refresh_channels(&sender);
            ...
            AppInput::PartChannel(ch) => {
                if ch == SERVER_TAB || !channels::is_channel_target(&ch) { return; }
                if let Some(tx) = &self.irc_sender { let _ = tx.send_part(&ch); }
            ...
            AppInput::ClearChannel(ch) => {
                ...
                if self.active_channel == ch { self.chat_view.buffer().set_text(""); }
            }
            AppInput::ToggleFavorite(ch) => {
                if self.favorite_channels.contains(&ch) { self.favorite_channels.retain(|c| c != &ch); }
                else { self.favorite_channels.push(ch.clone()); }
                self.refresh_channels(&sender);
                self.persist_settings();
            }
            AppInput::ToggleMute { channel, user } => {
                ...
                if list.contains(&user) {
                    ...
                    self.append_message(&channel, "System", &format!("Unmuted {}", user), chat_view::LineStyle::System);
                } else {
                    list.push(user.clone());
                    list.sort_by_key(|u| u.to_lowercase());
                    self.append_message(&channel, "System", &format!("Muted {}", user), chat_view::LineStyle::System);
                }
                if self.active_channel == channel { self.refresh_users(&sender); }
            }
            AppInput::ReceiveMessage { channel, user, body, protocol } => {
                let clean = Self::normalized_nick(&user);
                if self.ignored_users.contains(&clean) { return; }
                if self.muted_users.get(&channel).map(|v| v.contains(&clean)).unwrap_or(false) { return; }
            ...
            AppInput::MatrixConnected { user_id } => {
                ...
                self.append_message(SERVER_TAB, "System", &format!("Matrix connected as {}.", user_id), chat_view::LineStyle::System);
                self.refresh_channels(&sender);
            }
            AppInput::MatrixRoomJoined { room_id, room_name } => {
                self.matrix_rooms.insert(room_id, room_name.clone());
                self.chat_histories.entry(room_name.clone()).or_insert_with(Vec::new);
                self.refresh_channels(&sender);
            }
                        ...
                        if let Ok(id) = alias.parse::<matrix_sdk::ruma::OwnedRoomOrAliasId>() {
                            match c.inner.join_room_by_id_or_alias(&id, &[]).await {
                                Ok(room) => {
                                    ...
                                    s.input(AppInput::MatrixRoomJoined { room_id, room_name: name });
                                }
