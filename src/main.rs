                    ...
                    match command {
                        ...
                        "/nick" => {
                            if let Some(nick) = parts.next() {
                                if let Some(irc_tx) = &self.irc_sender { let _ = irc_tx.send(format!("NICK {}", nick)); }
                        ...
                        "/whois" => {
                            if let Some(target) = parts.next() {
                                if let Some(irc_tx) = &self.irc_sender { let _ = irc_tx.send(format!("WHOIS {}", target)); }
                            }
                            return;
                        }
                        "/away" => {
                            let msg = parts.next().unwrap_or("Away");
                            if let Some(irc_tx) = &self.irc_sender { let _ = irc_tx.send(format!("AWAY :{}", msg)); }
                            return;
                        }
                        "/back" => {
                            if let Some(irc_tx) = &self.irc_sender { let _ = irc_tx.send("AWAY"); }
                            return;
                        }
                        "/topic" => {
                            ...
                            let chan = self.active_channel.clone();
                            if let Some(irc_tx) = &self.irc_sender {
                                if new_topic.is_empty() { let _ = irc_tx.send(format!("TOPIC {}", chan)); }
                                else { let _ = irc_tx.send(format!("TOPIC {} :{}", chan, new_topic)); }
                            }
                            return;
                        }
                        "/ignore" => {
